...
function JSExecute(ast,option)
{
    ...
    if (option)
    {
        ...
        if (option.CallbackParam) 
        {
            ...
            if (this.CallbackParam.TKChart)
                this.GetEventCallback=(id)=> { return this.CallbackParam.TKChart.GetEventCallback(id); }
        }
        if (option.Arguments) this.Arguments=option.Arguments;
...
function ScriptIndex(name,script,args,option)
{
    ...
    this.ExecuteScript=function(tkChart,windowIndex,hisData)
    {
        this.OutVar=[];
        let self = this;
        let param=
        ...
        if (tkChart.ClassName==='MinuteChartContainer' || tkChart.ClassName==='MinuteChartHScreenContainer') 
        {
            if (tkChart.DayCount>1) hqDataType=HQ_DATA_TYPE.MULTIDAY_MINUTE_ID; //多日分钟
            else hqDataType=HQ_DATA_TYPE.MINUTE_ID;                             //分钟数据
        }
        else if (tkChart.ClassName==='HistoryMinuteChartContainer') 
        {
        ...
        let option=
        {
            ...
            Symbol:tkChart.Symbol, 
            Name:tkChart.Name,
            Data:hisData,
            SourceData:tkChart.SourceData,
            Callback:(arg1, arg2)=>{ this.RecvResultData(arg1, arg2); }, CallbackParam:param,
            Async:true,
            MaxRequestDataCount:tkChart.MaxRequestDataCount,
            MaxRequestMinuteDayCount:tkChart.MaxRequestMinuteDayCount,
            Arguments:this.Arguments,
            Condition:this.Condition,
            IsBeforeData:tkChart.IsBeforeData,
            IsApiPeriod:tkChart.IsApiPeriod,
            DrawInfo:null,
            Self:this,
        };
        ...
        if (tkChart)    //当前屏K线信息
        {
            if (tkChart.ChartPaint[0]) 
            {
                var item=tkChart.ChartPaint[0];
                if (item && item.DrawKRange) option.DrawInfo={Start:item.DrawKRange.Start, End:item.DrawKRange.End };
            }
        }

        if (hqDataType===HQ_DATA_TYPE.HISTORY_MINUTE_ID) option.TrateDate=tkChart.TradeDate;
        if (hqDataType===HQ_DATA_TYPE.MULTIDAY_MINUTE_ID) option.DayCount=tkChart.DayCount;
        if (tkChart.NetworkFilter) option.NetworkFilter=tkChart.NetworkFilter;
        ...
        let run=JSComplier.Execute(code,option,tkChart.ScriptErrorCallback);
    }
        ...
        for(var i in this.Condition.Period)
        {
            ...
            switch(item)
            {
                ...
                case CONDITION_PERIOD.KLINE_60_MINUTE_ID:
                    if (param.TKChart.Period==item) return true;
                    break;
            }
        }
    ...
    this.IsMeetIncludeCondition=function(param,option)
    {
        ...
        var symbol=param.TKChart.Symbol;
    ...
    {
        var tkChart=param.TKChart;
        var windowIndex=param.WindowIndex;

        tkChart.DeleteIndexPaint(windowIndex);
        if (windowIndex==0) tkChart.ShowKLine(true);

        var message='指标不支持当前品种或周期';
        if (msg) message=msg;

        let line=new ChartLine();
        line.Canvas=tkChart.Canvas;
        line.ChartBorder=tkChart.Frame.SubFrame[windowIndex].Frame.ChartBorder;
        line.ChartFrame=tkChart.Frame.SubFrame[windowIndex].Frame;
        line.NotSupportMessage=message;
        tkChart.ChartPaint.push(line);
        
        tkChart.UpdataDataoffset();           //更新数据偏移
        tkChart.UpdateFrameMaxMin();          //调整坐标最大 最小值
        tkChart.Draw();
    }

    this.RecvResultData=function(outVar,param)
    {
        let tkChart=param.TKChart;
        let windowIndex=param.WindowIndex;
        let hisData=param.HistoryData;
        param.Self.OutVar=outVar;
        param.Self.BindData(tkChart,windowIndex,hisData);

        if (param.Self.IsLocked==false) //不上锁
        {
            param.TKChart.Frame.SubFrame[windowIndex].Frame.SetLock(null);
        }
        else    //上锁
        {
            ...
            param.TKChart.Frame.SubFrame[windowIndex].Frame.SetLock(lockData);
        }

        param.TKChart.UpdataDataoffset();           //更新数据偏移
        param.TKChart.UpdateFrameMaxMin();          //调整坐标最大 最小值

        if (param.Self.IsSync===false)    //异步需要马上刷新，同步主图数据更新的时候会刷新的
            param.TKChart.Draw();

        if (tkChart.GetIndexEvent)
        {
            var event=tkChart.GetIndexEvent();  //指标计算完成回调
            if (event)
            {
                var self=param.Self;
                var data={ OutVar:self.OutVar, WindowIndex: windowIndex, Name: self.Name, Arguments: self.Arguments, HistoryData: hisData, 
                        Stock: {Symbol:tkChart.Symbol,Name:tkChart.Name} };
    ...
    //自定义图形配色
    this.ReloadChartResource=function(tkChart, windowIndex, chart)
    {
        var event=tkChart.GetEventCallback(JSCHART_EVENT_ID.ON_RELOAD_INDEX_CHART_RESOURCE);  //指标计算完成回调
        if (!event || !event.Callback) return;
        
        var sendData={ Chart:chart, IndexName:this.Name,IndexID:this.ID,  TKChart:tkChart, WindowIndex:windowIndex };
        event.Callback(event,sendData,this);
    }

    this.CreateLine=function(tkChart,windowIndex,varItem, id, lineType)
    {
        ...
        this.ReloadChartResource(tkChart,windowIndex,line);
        ...
        {
            if (varItem.NoneName) 
                tkChart.TitlePaint[titleIndex].Data[id]=new DynamicTitleData(line.Data,null,line.Color);
            else
                tkChart.TitlePaint[titleIndex].Data[id]=new DynamicTitleData(line.Data,varItem.Name,line.Color);

            this.SetTitleData(tkChart.TitlePaint[titleIndex].Data[id],line);
        }
    ...
    this.CreateArea=function(tkChart, windowIndex, varItem, id)
    {
        ...
        else
        {
            ...
            tkChart.TitlePaint[titleIndex].Data[id].ChartClassName=line.ClassName;
        }
    ...
    //标题显示，没有图形
    this.CreateTitle=function(tkChart, windowIndex, varItem, id)
    {
        var chart=new ChartIndexTitle();

        chart.Canvas=tkChart.Canvas;
        chart.Name=varItem.Name;
        chart.ChartBorder=tkChart.Frame.SubFrame[windowIndex].Frame.ChartBorder;
        chart.ChartFrame=tkChart.Frame.SubFrame[windowIndex].Frame;
        chart.Identify=this.Guid;
        chart.Data=tkChart.GetKData();                  //绑定K线
        ...
        tkChart.TitlePaint[titleIndex].Data[id]=titleData;

        this.SetChartIndexName(chart);
        tkChart.ChartPaint.push(chart);
    }

    this.CreateOverlayLine=function(tkChart,windowIndex,varItem,id,lineType)
    {
        if (lineType==7) var line=new ChartStepLine();
    ...
    this.CreateSingleLine=function(tkChart,windowIndex,varItem,id,lineType)
    {
    ...
    //创建柱子
    this.CreateBar=function(tkChart,windowIndex,varItem,id)
    {
        let bar=new ChartStickLine();
        bar.Canvas=tkChart.Canvas;
        if (varItem.Draw.Width>0) bar.Width=varItem.Draw.Width;
        else bar.Width=0;

        bar.Name=varItem.Name;
        bar.ChartBorder=tkChart.Frame.SubFrame[windowIndex].Frame.ChartBorder;
        bar.ChartFrame=tkChart.Frame.SubFrame[windowIndex].Frame;
        ...
        //tkChart.TitlePaint[titleIndex].Data[id]=new DynamicTitleData(bar.Data,varItem.Name,bar.Color);

        this.SetChartIndexName(bar);
        tkChart.ChartPaint.push(bar);
    }

    //创建文本
    this.CreateText=function(tkChart,windowIndex,varItem,id,resource)
    {
        let chartText=new ChartSingleText();
        chartText.Canvas=tkChart.Canvas;

        chartText.Name=varItem.Name;
        chartText.ChartBorder=tkChart.Frame.SubFrame[windowIndex].Frame.ChartBorder;
        chartText.ChartFrame=tkChart.Frame.SubFrame[windowIndex].Frame;
        ...
        tkChart.ChartPaint.push(chartText);
    }

    //DRAWNUMBER
    this.CreateDrawNumber=function(tkChart,windowIndex,varItem,id)
    {
    ...
    //DRAWTEXT
    this.CreateDrawTextV2=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateDrawFlagText=function(tkChart,windowIndex,varItem,id)
    {
    ...
    //COLORSTICK 
    this.CreateMACD=function(tkChart,windowIndex,varItem,id)
    {
        let chartMACD=new ChartMACD();
        chartMACD.Canvas=tkChart.Canvas;
        chartMACD.Identify=this.Guid;
        chartMACD.Name=varItem.Name;
        chartMACD.ChartBorder=tkChart.Frame.SubFrame[windowIndex].Frame.ChartBorder;
        chartMACD.ChartFrame=tkChart.Frame.SubFrame[windowIndex].Frame;
        if (varItem.LineWidth) 
        {
        ...
        this.ReloadChartResource(tkChart,windowIndex,chartMACD);

        tkChart.TitlePaint[titleIndex].Data[id]=new DynamicTitleData(chartMACD.Data,varItem.Name,clrTitle);

        this.SetChartIndexName(chartMACD);
        tkChart.ChartPaint.push(chartMACD);
    }

    this.CreatePointDot=function(tkChart,windowIndex,varItem,id, hisData)
    {
        let pointDot=new ChartPointDot();
        pointDot.Canvas=tkChart.Canvas;
        pointDot.Name=varItem.Name;
        pointDot.ChartBorder=tkChart.Frame.SubFrame[windowIndex].Frame.ChartBorder;
        pointDot.ChartFrame=tkChart.Frame.SubFrame[windowIndex].Frame;
        ...
        tkChart.TitlePaint[titleIndex].Data[id]=new DynamicTitleData(pointDot.Data,varItem.Name,pointDot.Color);

        this.SetChartIndexName(pointDot);
        tkChart.ChartPaint.push(pointDot);
    }

    this.CreateStick=function(tkChart,windowIndex,varItem,id)
    {
        ...
        this.ReloadChartResource(tkChart,windowIndex,chart);

        tkChart.TitlePaint[titleIndex].Data[id]=new DynamicTitleData(chart.Data,varItem.Name,chart.Color);

        this.SetChartIndexName(chart);
        tkChart.ChartPaint.push(chart);
    }

    this.CreateLineStick=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateStraightLine=function(tkChart,windowIndex,varItem,id)
    {
        let line=new ChartLine();
        line.DrawType=1;
        line.Canvas=tkChart.Canvas;
    ...
    this.CreateVolStick=function(tkChart,windowIndex,varItem,id,hisData)
    {
        ...
        chart.ChartBorder=tkChart.Frame.SubFrame[windowIndex].Frame.ChartBorder;
        chart.ChartFrame=tkChart.Frame.SubFrame[windowIndex].Frame;
        chart.KLineDrawType=tkChart.KLineDrawType;  //设置K线显示类型
        chart.Identify=this.Guid;
        if (varItem.Color) chart.Color=this.GetColor(varItem.Color);
        else chart.Color=this.GetDefaultColor(id);
    ...
    this.CreateBand=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateFillRGN=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateFillRGN2=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateFillBGRGN=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateFLOATRGN=function(tkChart,windowIndex,varItem,id)
    {
    ...
    //创建K线图
    this.CreateKLine=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateOverlayKLine=function(tkChart,windowIndex,varItem,id)
    {
        var chart=null;
        if (tkChart.IsKLineContainer()) chart=new ChartOverlayKLine();
        else if (tkChart.IsMinuteContainer()) chart=new ChartOverlayMinutePriceLine();
        else return;
        ...
        chart.Data.Data=varItem.Draw.DrawData;
        chart.MainData=tkChart.ChartPaint[0].Data;//绑定K线

        tkChart.ChartPaint.push(chart);
    }

    this.CreateDrawColorKLine=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreatePolyLine=function(tkChart,windowIndex,varItem,id)
    {
        let line=new ChartLine();
        line.Canvas=tkChart.Canvas;
        line.Name=varItem.Name;
        ...
        let titleIndex=windowIndex+1;
        line.Data.Data=varItem.Draw.DrawData;
        tkChart.TitlePaint[titleIndex].Data[id]=new DynamicTitleData(line.Data,' ',line.Color); //给一个空的标题

        tkChart.ChartPaint.push(line);
    }

    this.CreateChartSlopeLine=function(tkChart,windowIndex,varItem,id)
    {
        ...
        tkChart.TitlePaint[titleIndex].Data[id]=new DynamicTitleData(line.Data,null,line.Color); //给一个空的标题

        tkChart.ChartPaint.push(line);
    }

    this.CreateChartVericaltLine=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateChartHorizontalLine=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateBackgroud=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateBackgroundDiv=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateLineMultiData=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateSimpleTable=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateSimplePie=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateSimpleRadar=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateSimpleDoughnut=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateTradeIcon=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateTextLine=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateNumberText=function(tkChart,windowIndex,varItem,id)
    {
        let chartText=new ChartSingleText();
        chartText.Canvas=tkChart.Canvas;
    ...
    this.CreateDrawText_Fix=function(tkChart,windowIndex,varItem,id)
    {
        ...
        chartText.BuildCacheData(tkChart.GetKData(),varItem.Draw.DrawData);

        this.SetChartIndexName(chartText);
        tkChart.ChartPaint.push(chartText);
    }

    this.CreateDrawNumber_Fix=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateDrawText=function(tkChart,windowIndex,varItem,id)
    {
        let chartText=new ChartSingleText();
        chartText.Canvas=tkChart.Canvas;
        chartText.Name=varItem.Name;
    ...
    //创建图标
    this.CreateIcon=function(tkChart,windowIndex,varItem,id)
    {
        let chartText=new ChartSingleText();
        chartText.Canvas=tkChart.Canvas;
        chartText.TextAlign='center';
    ...
    this.CreateTipIcon=function(tkChart,windowIndex,varItem,i)
    {
        ...
        if (tkChart.ChartPaint[0].IsMinuteFrame())
            chart.Data=tkChart.SourceData;
        else
            chart.Data=tkChart.ChartPaint[0].Data;  //绑定K线
    ...
    this.CreateDrawLastBarText=function(tkChart,windowIndex,varItem,id)
    {
    ...
    //创建通道
    this.CreateChannel=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreatePartLine=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateMultiLine=function(tkChart,windowIndex,varItem,i)
    {
        ...
        if (bShowTitle)
        {
            ...
            tkChart.TitlePaint[titleIndex].Data[i]=titleData;
        }
    }

    this.CreateMultiPoint=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateMultiBar=function(tkChart,windowIndex,varItem,id)
    {
    ...
    this.CreateMultiText=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateMultiSVGIcon=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateChartDrawSVG=function(tkChart,windowIndex,varItem,i)
    {
        var chart=new ChartDrawSVG();
        chart.Canvas=tkChart.Canvas;
        chart.Name=varItem.Name;
    ...
    this.CreateMulitHtmlDom=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateStackedBar=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateKLineTable=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateScatterPlot=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateScatterPlotV2=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateBaseLineBar=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateClipColorStick=function(tkChart,windowIndex,varItem,id)
    {
        ...
        else
        {
            ...
            tkChart.TitlePaint[titleIndex].Data[id]=new DynamicTitleData(chart.Data,varItem.Name,clrTitle);
        }
    }

    this.CreateColorKLine=function(tkChart,windowIndex,varItem,i)
    {
        let chart=new ChartColorKline();
        chart.Canvas=tkChart.Canvas;
        chart.Name=varItem.Name;
    ...
    this.CreateRectangle=function(tkChart,windowIndex,varItem,i)
    {
    ...
    this.CreateScriptOverlayLine=function(tkChart,windowIndex,varItem,i)
    {
        ...
        if (varItem.Draw.DrawData.Title)
            tkChart.TitlePaint[titleIndex].Data[i]=new DynamicTitleData(chart.Data,varItem.Draw.DrawData.Title,chart.Color);

        tkChart.ChartPaint.push(chart);
    }

    //创建K线
    this.CreateSelfKLine=function(tkChart,windowIndex,hisData)
    {
        let chart=new ChartKLine();
        chart.Canvas=tkChart.Canvas;
    ...
    this.BindInstructionData=function(tkChart,windowIndex,hisData)  //绑定指示指标
    {
        ...
        if (this.InstructionType==2)        //五彩K线
        {
            ...
            tkChart.SetInstructionData(this.InstructionType, {Data:varItem.Data, Name:this.Name, Param:indexParam, ID:this.ID });       //设置指示数据
            return true;
        }
        else if (this.InstructionType==1)   //交易系统
        {
            ...
            tkChart.SetInstructionData(this.InstructionType, {Buy:buyData, Sell:sellData, Name:this.Name, Param:indexParam, ID:this.ID});       //设置指示数据
            return true;
        }
    }

    this.BindData=function(tkChart,windowIndex,hisData)
    {
        if (windowIndex==0 && this.InstructionType)
        {
            this.BindInstructionData(tkChart,windowIndex,hisData);
            return;
        }
        ...
        {
            if (this.KLineType===0 || this.KLineType===1 || this.KLineType===2) this.CreateSelfKLine(tkChart,windowIndex,hisData);
            else if (this.KLineType===-1 && windowIndex==0) tkChart.ShowKLine(false);
        }

        if (windowIndex>=1 && tkChart.Frame)
        {
            //Y轴刻度格式 默认跟标题栏一致
            var ySpliter=tkChart.Frame.SubFrame[windowIndex].Frame.YSplitOperator;
            if (ySpliter)
            {
                ySpliter.Reset();
            ...
            if (this.YSpecificMaxMin)  tkChart.Frame.SubFrame[windowIndex].Frame.YSpecificMaxMin=this.YSpecificMaxMin;  //最大最小值
            if (this.YSplitScale)   tkChart.Frame.SubFrame[windowIndex].Frame.YSplitScale=this.YSplitScale;             //固定刻度
        }

        if (IFrameSplitOperator.IsNumber(this.YSplitType)) tkChart.Frame.SubFrame[windowIndex].Frame.YSplitOperator.SplitType=this.YSplitType;
        ...
        */
        ...
        for(let i=0; i<this.OutVar.length; ++i)
        {
            ...
            if (item.Type==0)  
            {
                if (item.IsOverlayLine) this.CreateOverlayLine(tkChart,windowIndex,item,i,item.Type);
                else if (item.IsSingleLine) this.CreateSingleLine(tkChart,windowIndex,item,i,item.Type);
                else this.CreateLine(tkChart,windowIndex,item,i,item.Type);
            }
            else if (item.Type==1)
            {
                ...
                {
                    case 'STICKLINE':
                        this.CreateBar(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWTEXT':
                        this.CreateDrawTextV2(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWFLAGTEXT":
                        this.CreateDrawFlagText(tkChart,windowIndex,item,i);
                        break;
                    case 'SUPERDRAWTEXT':
                        this.CreateText(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWLINE':
                        this.CreateStraightLine(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWBAND':
                        this.CreateBand(tkChart,windowIndex,item,i);
                        break;
                    case "FILLRGN":
                        this.CreateFillRGN(tkChart,windowIndex,item,i);
                        break;
                    case "FILLRGN2":
                        this.CreateFillRGN2(tkChart,windowIndex,item,i);
                        break;
                    case "FILLTOPRGN":
                    case "FILLBOTTOMRGN":
                    case "FILLVERTICALRGN":
                        this.CreateFillBGRGN(tkChart,windowIndex,item,i);
                        break;
                    case "FLOATRGN":
                        this.CreateFLOATRGN(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWKLINE':
                    case "DRAWKLINE1":
                        this.CreateKLine(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWOVERLAYKLINE":
                        this.CreateOverlayKLine(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWCOLORKLINE":
                        this.CreateDrawColorKLine(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWKLINE_IF':
                        this.CreateKLine(tkChart,windowIndex,item,i);
                        break;
                    case 'POLYLINE':
                        this.CreatePolyLine(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWGBK':
                    case "DRAWGBK2":
                        this.CreateBackgroud(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWGBK_DIV":
                        this.CreateBackgroundDiv(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWTEXT_LINE':
                        this.CreateTextLine(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWNUMBER':
                        this.CreateDrawNumber(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWNUMBER_FIX":
                        this.CreateDrawNumber_Fix(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWTEXT_FIX':
                        this.CreateDrawText_Fix(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWICON':
                        this.CreateIcon(tkChart,windowIndex,item,i);
                        break;
                    case "ICON":
                        this.CreateIcon(tkChart,windowIndex,item,i);
                        break;
                    case "TIPICON":
                        this.CreateTipIcon(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWCHANNEL':
                        this.CreateChannel(tkChart,windowIndex,item,i);
                        break;
                    case 'PARTLINE':
                        this.CreatePartLine(tkChart,windowIndex,item,i);
                        break;
                    case 'DRAWRECTREL':
                        this.CreateRectangle(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWTEXTABS":
                    case "DRAWTEXTREL":
                        this.CreateDrawText(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWOVERLAYLINE":
                        this.CreateScriptOverlayLine(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWSL":
                        this.CreateChartSlopeLine(tkChart,windowIndex,item,i);
                        break;
                    case "VERTLINE":
                        this.CreateChartVericaltLine(tkChart,windowIndex,item,i);
                        break;
                    case "HORLINE":
                        this.CreateChartHorizontalLine(tkChart,windowIndex,item,i);
                        break;
                    case 'MULTI_LINE':
                        this.CreateMultiLine(tkChart,windowIndex,item,i);
                        break;
                    case "MULTI_POINT":
                        this.CreateMultiPoint(tkChart,windowIndex,item,i);
                        break;
                    case 'MULTI_BAR':
                        this.CreateMultiBar(tkChart,windowIndex,item,i);
                        break;
                    case 'MULTI_TEXT':
                        this.CreateMultiText(tkChart,windowIndex,item,i);
                        break;
                    case 'MULTI_SVGICON':
                        this.CreateMultiSVGIcon(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWSVG":
                        this.CreateChartDrawSVG(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWTITLE":
                        this.CreateTitle(tkChart,windowIndex,item,i);
                        break;
                    case "MULTI_HTMLDOM":
                        this.CreateMulitHtmlDom(tkChart,windowIndex,item,i);
                        break;
                    case "COLOR_KLINE":
                        this.CreateColorKLine(tkChart,windowIndex,item,i);
                        break;
                    case "KLINE_BG":
                        this.CreateBackgroud(tkChart,windowIndex,item,i);
                        break;
                    case "MULTI_POINT_LINE":
                        this.CreateLineMultiData(tkChart,windowIndex,item,i);
                        break;
                    case "DRAW_SIMPLE_TABLE":
                        this.CreateSimpleTable(tkChart,windowIndex,item,i);
                        break;
                    case "DRAW_SIMPLE_PIE":
                        this.CreateSimplePie(tkChart,windowIndex,item,i);
                        break;
                    case "DRAW_SIMPLE_RADAR":
                        this.CreateSimpleRadar(tkChart,windowIndex,item,i);
                        break;
                    case "DRAW_SIMPLE_DOUGHNUT":
                        this.CreateSimpleDoughnut(tkChart,windowIndex,item,i);
                        break;
                    case "BUY":
                    case "SELL":
                    case "SELLSHORT":
                    case "BUYSHORT":
                        this.CreateTradeIcon(tkChart,windowIndex,item,i);
                        break;

                    case "DRAWLASTBARICON":
                    case "DRAWLASTBARNUMBER":
                    case "DRAWLASTBARTEXT":
                        this.CreateDrawLastBarText(tkChart,windowIndex,item,i);
                        break;

                    case SCRIPT_CHART_NAME.OVERLAY_BARS:
                        this.CreateStackedBar(tkChart,windowIndex,item,i);
                        break;
                    case SCRIPT_CHART_NAME.KLINE_TABLE:
                        this.CreateKLineTable(tkChart,windowIndex,item,i);
                        break;
                    case SCRIPT_CHART_NAME.SCATTER_PLOT:
                        this.CreateScatterPlot(tkChart,windowIndex,item,i);
                        break;
                    case SCRIPT_CHART_NAME.SCATTER_PLOT_V2:
                        this.CreateScatterPlotV2(tkChart,windowIndex,item,i);
                        break;
                    case SCRIPT_CHART_NAME.CLIP_COLOR_STICK:
                        this.CreateClipColorStick(tkChart,windowIndex,item,i);
                        break;
                    case SCRIPT_CHART_NAME.BASELINE_BAR:
                        this.CreateBaseLineBar(tkChart,windowIndex,item,i);
                        break;
                    default:
                        {
                            var find=g_ScriptIndexChartFactory.Get(item.Draw.DrawType);  //外部挂接
                            if (find && find.CreateChartCallback)
                                find.CreateChartCallback(tkChart,windowIndex,item,i, this);
                        }
                        break;
                }
            }
            ...
            {
                this.CreateMACD(tkChart,windowIndex,item,i);
            }
            else if (item.Type==3)
            {
                this.CreatePointDot(tkChart,windowIndex,item,i, hisData);
            }
            else if (item.Type==4)
            {
                this.CreateLineStick(tkChart,windowIndex,item,i);
            }
            else if (item.Type==5)
            {
                this.CreateStick(tkChart,windowIndex,item,i);
            }
            else if (item.Type==6)
            {
                this.CreateVolStick(tkChart,windowIndex,item,i,hisData);
            }
            else if (item.Type==7)
            {
                this.CreateLine(tkChart,windowIndex,item,i, item.Type);
            }
            else if (item.Type==8)
            {
                this.CreateLine(tkChart,windowIndex,item,i, item.Type);
            ...
            {
                this.CreateArea(tkChart,windowIndex,item,i);
            }
        
            var titlePaint=tkChart.TitlePaint[windowIndex+1];
        ...
        let titleIndex=windowIndex+1;
        var titlePaint=tkChart.TitlePaint[titleIndex];
...
function OverlayScriptIndex(name,script,args,option)
{
    ...
    this.BindData=function(tkChart,windowIndex,hisData)
    {
        ...
        if (tkChart.Frame.SubFrame[windowIndex])
        {
            var mainFrame=tkChart.Frame.SubFrame[windowIndex].Frame;
            ...
            else if (item.Type==1)
            {
                switch(item.Draw.DrawType)
                {
                    ...
                    case "DRAWGBK2":
                        ...
                        break;
                    case 'DRAWNUMBER':
                        this.CreateNumberText(tkChart,windowIndex,item,i);
                        break;
                    case "DRAWNUMBER_FIX":  
                        this.CreateDrawNumber_Fix(tkChart,windowIndex,item,i);
                        break;
        ...
        /*
        tkChart.TitlePaint[titleIndex].Title=this.Name;
        ...
        if (indexParam.length>0) tkChart.TitlePaint[titleIndex].Title=this.Name+'('+indexParam+')';
        */
    ...
    this.RecvResultData=function(outVar,param)
    {
        ...
        var event=tkChart.GetOverlayIndexEvent();  //指标计算完成回调
        if (event)
        {
            var self=param.Self;
            var data={ OutVar:self.OutVar, WindowIndex: windowIndex, Name: self.Name, Arguments: self.Arguments, HistoryData: hisData, 
    ...
    this.ReloadChartResource=function(tkChart, windowIndex, chart)
    {
        var event=tkChart.GetEventCallback(JSCHART_EVENT_ID.ON_RELOAD_OVERLAY_INDEX_CHART_RESOURCE);  //指标计算完成回调
        if (!event || !event.Callback) return;
        ...
        var sendData={ Chart:chart, IndexName:script.Name,IndexID:script.ID, TKChart:tkChart, WindowIndex:windowIndex, Guid:overlayIndex.Identify };
        event.Callback(event,sendData,this);
    }
    ...
    //创建文本
    this.CreateText=function(tkChart,windowIndex,varItem,id, drawName)
    {
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
    ...
    this.CreateDrawText_Fix=function(tkChart,windowIndex,varItem,id)
    {
        ...
        chart.BuildCacheData(tkChart.GetKData(),varItem.Draw.DrawData);
    ...
    this.CreateStackedBar=function(tkChart,windowIndex,varItem,id)
    {
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
    ...
    this.CreateDrawColorKLine=function(tkChart,windowIndex,varItem,id)
    {
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
        let chart=new ChartColorKline();
    ...
    this.CreateTipIcon=function(tkChart,windowIndex,varItem,id)
    {
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
        var chart=new ChartDrawSVG();
    ...
    this.CreateBaseLineBar=function(tkChart,windowIndex,varItem,id)
    {
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
        var chart=new ChartBaseLineBar();
    ...
    this.CreateKLineTable=function(tkChart,windowIndex,varItem,id)
    {
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
        var chart=new ChartKLineTable();
    ...
    this.CreateMultiPoint=function(tkChart,windowIndex,varItem,id)
    {
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
        let chart=new ChartMultiPoint();
    ...
    this.CreateBackgroud=function(tkChart,windowIndex,varItem,i)
    {
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
        let chart=new ChartBackground();
    ...
    this.CreateSimpleTable=function(tkChart,windowIndex,varItem,i)
    {
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
        var chart=new ChartSimpleTable();
...
function APIScriptIndex(name,script,args,option, isOverlay)
{
    ...
    //接收到订阅指标数据
    this.RecvSubscribeData=function(data, tkChart, windowIndex, hisData)
    {
        if (this.Version==2)
            this.RecvAPIData2(data,tkChart,windowIndex,hisData);
        else 
            this.RecvAPIData(data,tkChart,windowIndex,hisData);
    }
    ...
    this.ExecuteScript=function(tkChart,windowIndex,hisData)
    {
        ...
        var requestCount;
        if (tkChart.GetRequestDataCount) requestCount=tkChart.GetRequestDataCount();
        var self=this;
        var postData =
        { 
            indexname:this.ID,  symbol: tkChart.Symbol, script:this.Script, args:args,
            period:tkChart.Period, right:tkChart.Right, hqdatatype: hqDataType
        };
        ...
        if (hqDataType==HQ_DATA_TYPE.MULTIDAY_MINUTE_ID || hqDataType==HQ_DATA_TYPE.MINUTE_ID) postData.daycount=tkChart.DayCount;
        this.HQDataType=hqDataType;

        ++this.RunCount;

        if (tkChart.NetworkFilter)
        {
            var obj=
            {
            ...
            tkChart.NetworkFilter(obj, function(data) 
            { 
                if (self.Version==2)
                    self.RecvAPIData2(data,tkChart,windowIndex,hisData);
                else 
                    self.RecvAPIData(data,tkChart,windowIndex,hisData);
            });
        ...
        JSNetwork.HttpRequest({
            ...
            success: function (recvData)
            {
                self.RecvAPIData(recvData,tkChart,windowIndex,hisData);
            },
            error: function(request)
            {
                self.RecvError(request);
    ...
    //py 后台指标计算格式
    this.RecvAPIData2=function(data,tkChart,windowIndex,hisData)
    {
        ...
        {
            this.OutVar=this.ConvertToLocalData(data,tkChart);
        ...
        this.BindData(tkChart,windowIndex,hisData);
    ...
    this.ConvertToLocalData=function(jsonData, tkChart)
    {
        var outVar=jsonData.OutVar;
        if (tkChart.ClassName=="MinuteChartContainer" || tkChart.ClassName=="MinuteChartHScreenContainer")
        {
            var kdata=tkChart.SourceData;
            var aryDataIndex=kdata.GetAPIDataIndex(jsonData.Date,jsonData.Time);
        }
        else
        { 
            var kdata=tkChart.ChartPaint[0].Data;
    ...
    this.RecvAPIData=function(data,tkChart,windowIndex,hisData)
    {
        JSConsole.Complier.Log('[APIScriptIndex::RecvAPIData] recv data ', this.Name,data );
        if (data.code!=0) return;

        if (tkChart.EnableVerifyRecvData)
        {
            if (!data.stock || tkChart.Symbol!=data.stock.symbol) 
            {
                JSConsole.Chart.Warn(`[APIScriptIndex::RecvAPIData] recv data symbol not match. TKChart[${tkChart.Symbol}]`);
                return;
            }
        }
        ...
        if (this.HQDataType==HQ_DATA_TYPE.KLINE_ID)
        {
            this.OutVar=this.FittingData(data.outdata,tkChart);
            JSConsole.Complier.Log('[APIScriptIndex::RecvAPIData] conver to OutVar ', this.OutVar);
        }
        else
        {
            this.OutVar=this.FittingMinuteData(data.outdata,tkChart);   //走势图数据
        }
        this.BindData(tkChart,windowIndex,hisData);

        var frame=tkChart.Frame.SubFrame[windowIndex].Frame;
    ...
    this.FittingArray=function(sourceData,date,time,tkChart,arrayType)  //arrayType 0=单值数组 1=结构体
    {
        var kdata=tkChart.ChartPaint[0].Data;   //K线
        ...
        var aryFittingData;
        if (ChartData.IsDayPeriod(tkChart.Period,true))
            aryFittingData=kdata.GetFittingData(arySingleData);        //数据和主图K线拟合
        else
            aryFittingData=kdata.GetMinuteFittingData(arySingleData);  //数据和主图K线拟合
    ...
    this.FittingMultiLine=function(sourceData,date,time,tkChart)
    {
        var kdata=tkChart.ChartPaint[0].Data;   //K线

        if (ChartData.IsDayPeriod(tkChart.Period,true))  //日线
        {
            var kdata=tkChart.ChartPaint[0].Data;   //K线
        ...
        else if (ChartData.IsMinutePeriod(tkChart.Period,true) || ChartData.IsTickPeriod(tkChart.Period) || ChartData.IsSecondPeriod(tkChart.Period)) //分钟线
        {
            var kdata=tkChart.ChartPaint[0].Data;   //K线
        ...
        {
            var minuteData=tkChart.SourceData;
    ...
    this.FittingMultiText=function(sourceData,date,time,tkChart)
    {
        if (ChartData.IsDayPeriod(tkChart.Period,true))  //日线
        {
            var kdata=tkChart.ChartPaint[0].Data;   //K线
        ...
        else if (ChartData.IsMinutePeriod(tkChart.Period,true) || ChartData.IsTickPeriod(tkChart.Period) || ChartData.IsSecondPeriod(tkChart.Period) || ChartData.IsMilliSecondPeriod(tkChart.Period)) //分钟线
        {
            var kdata=tkChart.ChartPaint[0].Data;   //K线
    ...
    this.FittingData=function(jsonData, tkChart)
    {
        var outVar=jsonData.outvar;
        ...
        for(var i=0;i<outVar.length; ++i)
        {
            ...
            if (item.data) 
            {
                outVarItem.Data=this.FittingArray(item.data,date,time,tkChart);
            ...
            else if (item.Draw)
            {
                ...
                if (draw.DrawType=='DRAWICON')  //图标
                {
                    ...
                    drawItem.DrawData=this.FittingArray(draw.DrawData,date,time,tkChart);
                ...
                else if (draw.DrawType=="DRAWTEXT_FIX" || draw.DrawType=="DRAWNUMBER_FIX")
                {
                    ...
                    drawItem.DrawData.Value=this.FittingArray(draw.DrawData.Value,date,time,tkChart);
                    drawItem.DrawData.Text=this.FittingArray(draw.DrawData.Text,date,time,tkChart);
                ...
                else if (draw.DrawType=="VERTLINE") //竖线
                {
                    ...
                    drawItem.Name=draw.Name;
                    drawItem.DrawType=draw.DrawType;
                    drawItem.DrawData={ Data:this.FittingArray(draw.DrawData.Data,date,time,tkChart) };
                ...
                else if (draw.DrawType=='STICKLINE')    //柱子
                {
                    ...
                    drawItem.DrawType=draw.DrawType;
                    drawItem.DrawData=this.FittingArray(draw.DrawData,date,time,tkChart,1);
                    outVarItem.Draw=drawItem;
                ...
                else if (draw.DrawType=='MULTI_LINE')
                {
                    ...
                    drawItem.Name=draw.Name;
                    drawItem.DrawType=draw.DrawType;
                    drawItem.DrawData=this.FittingMultiLine(draw.DrawData,date,time,tkChart);
                    if (IFrameSplitOperator.IsNonEmptyArray(drawItem.DrawData))
                    {
                        for(var k=0; k<drawItem.DrawData.length; ++k)
                        {
                            this.GetKLineData(drawItem.DrawData[k].Point, tkChart);
                        }
                    }
                ...
                else if (draw.DrawType=="MULTI_HTMLDOM")    //外部自己创建dom
                {
                    ...
                    drawItem.DrawData=this.FittingMultiText(draw.DrawData,date,time,tkChart);
                    this.GetKLineData(drawItem.DrawData, tkChart);
                    outVarItem.Draw=drawItem;
                ...
                else if (draw.DrawType=="COLOR_KLINE")
                {
                    ...
                    drawItem.Name=draw.Name;
                    drawItem.DrawType=draw.DrawType;
                    var klineOption=this.FittingMultiText(draw.DrawData.KLine,date,time,tkChart);
                ...
                else if (draw.DrawType=="KLINE_BG")
                {
                    ...
                    if (draw.Ver==2.0) drawItem.DrawData.Data=this.FittingKLineBG_V2(draw.DrawData, tkChart);
                    else drawItem.DrawData.Data=this.FittingKLineBG(draw.DrawData, tkChart);
                ...
                else if (draw.DrawType=='MULTI_POINT_LINE')
                {
                    ...
                    drawItem.DrawData.Data=this.FittingMultiPointLine(draw.DrawData, tkChart);
                ...
                else if (draw.DrawType==SCRIPT_CHART_NAME.CLIP_COLOR_STICK)
                {
                    ...
                    drawItem.Type=draw.Type;
                    drawItem.DrawType=draw.DrawType;
                    drawItem.DrawData=this.FittingArray(draw.Data,date,time,tkChart);
                ...
                else
                {
                    ...
                    {
                        if (find.KLineFittingCallback(item, outVarItem, { Date:date, Time:time, TKChart:tkChart }, this)) 
                            result.push(outVarItem);
                    }
                }
            }
    ...
    this.FittingKLineBG=function(data, tkChart)
    {
        var kData=tkChart.ChartPaint[0].Data;   //K线
        var result=[];
        if (ChartData.IsDayPeriod(tkChart.Period,true))  //日线
        ...
        else if (ChartData.IsMinutePeriod(tkChart.Period,true)) //分钟线
        {
            var bFill=false;
            for(var i=0,j=0;i<kData.Data.length;)
            {
    ...
    this.FittingKLineBG_V2=function(data, tkChart)  //2.0版本通过  [{ Start:, End: }, .....]来填充背景色
    {
        var kData=tkChart.ChartPaint[0].Data;   //K线
        var result=[];
        if (ChartData.IsDayPeriod(tkChart.Period,true))  //日线
    ...
    this.FittingMinuteData=function(jsonData, tkChart)
    {
        var outVar=jsonData.outvar;
        var date=jsonData.date;
        var time=jsonData.time;
        ...
        for(var i=0;i<outVar.length;++i)
        {
            ...
            if (item.data)
            {
                outVarItem.Data=this.FittingMinuteArray(item.data,date,time,tkChart);
                if (item.color) outVarItem.Color=item.color;
                if (item.linewidth) outVarItem.LineWidth=item.linewidth;
            ...
            else if (item.Draw)
            {
                ...
                if (draw.DrawType=='DRAWICON')  //图标
                {
                    ...
                    drawItem.Name=draw.Name;
                    drawItem.DrawType=draw.DrawType;
                    drawItem.DrawData=this.FittingMinuteArray(draw.DrawData,date,time,tkChart);
                    outVarItem.Draw=drawItem;
                ...
                else if (draw.DrawType=='STICKLINE')    //柱子
                {
                    ...
                    drawItem.Width=draw.Width;
                    drawItem.DrawType=draw.DrawType;
                    drawItem.DrawData=this.FittingMinuteArray(draw.DrawData,date,time,tkChart,1);
                    outVarItem.Draw=drawItem;
                ...
                else if (draw.DrawType==SCRIPT_CHART_NAME.CLIP_COLOR_STICK)
                {
                    ...
                    drawItem.Type=draw.Type;
                    drawItem.DrawType=draw.DrawType;
                    drawItem.DrawData=this.FittingMinuteArray(draw.Data,date,time,tkChart);
                ...
                else
                {
                    ...
                    {
                        if (find.MinuteFittingCallback(item, outVarItem, { Date:date, Time:time, TKChart:tkChart }, this)) 
                            result.push(outVarItem);
                    }
                }
            }
    ...
    this.FittingMinuteArray=function(sourceData,date,time,tkChart,matchType)
    {
        var minutedata=tkChart.SourceData;;   //分钟线
    ...
    this.FittingMultiPointLine=function(data, tkChart)
    {
        var period=tkChart.Period;              //周期
...
function LocalJsonDataIndex(name,args,option)
{
    ...
    this.RequestData=function(tkChart,windowIndex,hisData)
    {
        if (!this.JsonData)
        {
            console.warn("[LocalJsonDataIndex::RequestData] JsonData is null");
            if (param.TKChart.ScriptErrorCallback) param.TKChart.ScriptErrorCallback('json 数据不能为空');
            return;
        }
...
function DownloadFinanceData(obj)
{
    ...
    this.DownloadRealtimeData=function(id)
    {
        ...
        JSNetwork.HttpRequest({
            ...
            success: function (recvData)
            {
                var data=self.RealtimeDataToTKChartData(recvData);
    ...
    this.DownloadHistoryData=function(id)
    {
        ...
        JSNetwork.HttpRequest({
            ...
            success: function (recvData)
            {
                var data=self.ToTKChartData(recvData);
                if (data) //排序
    ...
    //最新报告期数据
    this.RealtimeDataToTKChartData=function(recvData,id)
    {
    ...
    //历史数据转
    this.ToTKChartData=function(recvData)
    {
        if (!recvData.stock || recvData.stock.length!=1) return null;
        ...
        for(var i in stock.stockday)
        {
            ...
            var tkchartItem=this.ToTKChartItemData(item,id);
            if (tkchartItem && !setDate.has(tkchartItem.Date)) 
            {
                aryData.push(tkchartItem);
                setDate.add(tkchartItem.Date);
            }
        }
    ...
    this.ToTKChartItemData=function(item, id)
    {
        if (!item) return null;
        var date=item.date;
        switch(id)
