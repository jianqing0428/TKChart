...
function JSDialogTooltip()
{
    ...
    this.ValueAlign=
    {
        Right:"IYXChart_Tooltip_Text_Span",
    };

    this.TitleAlign=
    {
        Right:"IYXChart_Tooltip_Title_Span",
        Left:"IYXChart_Tooltip_Title_Left_Span",
        Center:"IYXChart_Tooltip_Title_Center_Span",
    };
    ...
    this.Inital=function(tkchart, option)
    {
    ...
    this.OnClickColseButton=function(e)
    {
        ...
        if (this.TKChart && this.TKChart.ChartCorssCursor)
        {
            var chart=this.TKChart.ChartCorssCursor;
            if (!chart.IsShowCorss) return;

            chart.IsShowCorss=false;
            this.TKChart.Draw();
            this.TKChart.SetFocus();
        }
    }
    ...
    this.Create=function()
    {
        ...
        divDom.className='IYXChart_Tooltip_Dialog_Div';

        var divTitle=document.createElement("div");
        divTitle.className='IYXChart_Tooltip_Title_Div';
        divTitle.onmousedown=(e)=>{   this.OnMouseDownTitle(e);}

        var divName=document.createElement("div");
        divName.className='IYXChart_Tooltip_Name_Div';
        divName.innerText="----";
        divTitle.appendChild(divName);

        var divClose=document.createElement("div");
        divClose.className='IYXChart_Tooltip_Close_Div';
        ...
        table.className="IYXChart_Tooltip_Table";
        divDom.appendChild(table);

        var tbody=document.createElement("tbody");
        tbody.className="IYXChart_Tooltip_Tbody";
        ...
        if (this.Style==1)
        {
            for(var i=0;i<this.MaxRowCount;++i)
            {
                ...
                trDom.className='IYXChart_Tooltip_Group_Tr';
                tbody.appendChild(trDom);
                rowItem.Tr=trDom;

                var tdDom=document.createElement("td");
                tdDom.className="IYXChart_Tooltip_Text_Sinlge_Td";   //标题
                trDom.appendChild(tdDom);

                var spanDom=document.createElement("span");
                spanDom.className='IYXChart_Tooltip_Title_Left_Span';
                ...
                var tdDom=document.createElement("td");
                tdDom.className="IYXChart_Tooltip_Text_Sinlge_Td";    //数值
                trDom.appendChild(tdDom);

                var spanDom=document.createElement("span");
                spanDom.className='IYXChart_Tooltip_Text_Span';
        ...
        else
        {
            for(var i=0;i<this.MaxRowCount;++i)
            {
                ...
                var tdDom=document.createElement("td");
                tdDom.className="IYXChart_Tooltip_Title_Td";   //标题
                trDom.appendChild(tdDom);
                rowItem.TitleTd=tdDom;

                var spanDom=document.createElement("span");
                spanDom.className='IYXChart_Tooltip_Title_Span';
                spanDom.innerText='标题';
                tdDom.appendChild(spanDom);
                rowItem.TitleSpan=spanDom;

                var tdDom=document.createElement("td");
                tdDom.className="IYXChart_Tooltip_Text_Td";    //数值
                trDom.appendChild(tdDom);
    ...
    this.UpdateKLineData=function(data)
    {
        ...
        this.LanguageID=this.TKChart.LanguageID;

        if (this.TKChart.ClassName=='KLineChartContainer')
        {
    ...
    this.Update=function(data)
    {
        ...
        else if (this.TKChart.ClassName=='MinuteChartContainer')
        {
            var strKItem=JSON.stringify(data.MinItem);
            var strLastValue=JSON.stringify(data.LastValue);
            var bUpdata=false;
    ...
    this.Show=function(x, y)
    {
        ...
        var scrollPos=GetScrollPosition();
        var top=this.TKChart.Frame.ChartBorder.GetTop()+scrollPos.Top;
        var left=this.TKChart.Frame.ChartBorder.GetLeft();
        var rtClient=this.TKChart.UIElement.getBoundingClientRect();
    ...
    {
        var defaultfloatPrecision=GetfloatPrecision(this.TKChart.Symbol);//价格小数位数
        var upperSymbol=this.TKChart.Symbol.toUpperCase();
        ...
        //时间
        var timeItem=null;
        if (IFrameSplitOperator.IsNumber(data.Time)) timeItem=this.FormatTime(data.Time, this.TKChart.Period, null, 'DialogTooltip-Time');
        ...
        var event=this.TKChart.GetEventCallback(JSCHART_EVENT_ID.ON_FORMAT_DIALOG_TOOLTIP);
        if (event && event.Callback)
        {
            var sendData={ AryText:aryText, Data:data, Symbol:this.TKChart.Symbol, TKChart:this.TKChart, IsKLine:true };
    ...
    this.GetIndexTitleTooltipText=function(kItem, data)
    {
        ...
        if (!IFrameSplitOperator.IsNonEmptyArray(this.TKChart.TitlePaint)) return null;
        if (!IFrameSplitOperator.IsNumber(data.DataIndex) || data.DataIndex<0) return null;

        var aryText=[];
        var dataIndex=data.DataIndex;
        for(var i=0,j=0,k=0; i<this.TKChart.TitlePaint.length && k<this.IndexTitle.MaxCount; ++i)
        {
            var titleChart=this.TKChart.TitlePaint[i];
    ...
    this.GetFormatMinuteTooltipText=function(data)
    {
        ...
        if (event && event.Callback)
        {
            var sendData={ AryText:aryText, Data:data, Symbol:this.TKChart.Symbol, TKChart:this.TKChart, IsMinute:true };
            event.Callback(event, sendData, this);
        }
...
function JSFloatTooltip()
{
    ...
    this.ValueAlign=
    {
        Left:"IYXChart_Tooltip_Float_Text2_Span",       //左对齐
        MarginLeft:'IYXChart_Tooltip_Float_Text3_Span',
        Right:"IYXChart_Tooltip_Float_Text_Span",
    }

    this.TitleAlign=
    {
        Default:"IYXChart_Tooltip_Float_Title_Span",   //标题默认
    }
    ...
    {
        var divDom=document.createElement("div");
        divDom.className='IYXChart_Tooltip_Float_Div';

        var table=document.createElement("table");
        table.className="IYXChart_Tooltip_Float_Table";
        divDom.appendChild(table);

        var tbody=document.createElement("tbody");
        tbody.className="IYXChart_Tooltip_Float_Tbody";
        table.appendChild(tbody);
        ...
        for(var i=0;i<this.MaxRowCount;++i)
        {
            ...
            var trDom=document.createElement("tr");
            trDom.className='IYXChart_Tooltip_Float_Group_Tr';
            tbody.appendChild(trDom);
            rowItem.Tr=trDom;

            var tdDom=document.createElement("td");
            tdDom.className="IYXChart_Tooltip_Float_Title_Td";   //标题
            trDom.appendChild(tdDom);
            rowItem.TitleTd=tdDom;
            ...
            var tdDom=document.createElement("td");
            tdDom.className="IYXChart_Tooltip_Float_Text_Td";    //数值
            trDom.appendChild(tdDom);
            rowItem.TextTd=tdDom;

            var spanDom=document.createElement("span");
            spanDom.className='IYXChart_Tooltip_Float_Text_Span';
            spanDom.innerText='数值';
            tdDom.appendChild(spanDom);
            rowItem.TextSpan=spanDom;
    ...
    this.UpdatChartOXTooltip=function(data)
    {
        ...
        var item=tooltipData.Data.Data;
        var period=this.TKChart.Period;
        var aryText=[];
    ...
    {
        if (!this.TKChart) return false;
        if (!data || !data.Tooltip || !data.Tooltip.Data) return false;
        if (!(data.Tooltip.Data.EnableCallback===true)) return false;
        
        var event=this.TKChart.GetEventCallback(JSCHART_EVENT_ID.ON_REPORT_SHOW_TOOLTIP);
    ...
    this.GetFormatKLineTooltipText=function(kItem)
    {
        ...
        //时间
        var timeItem=null;
        if (IFrameSplitOperator.IsNumber(data.Time)) timeItem=this.FormatTime(data.Time, this.TKChart.Period, null, 'FloatTooltip-Time');
        ...
        var event=this.TKChart.GetEventCallback(JSCHART_EVENT_ID.ON_FORMAT_KLINE_FLOAT_TOOLTIP);
        if (event && event.Callback)
        {
            var sendData={ AryText:aryText, Data:kItem, TKChart:this.TKChart };
            event.Callback(event, sendData, this);
        }
    ...
    this.UpdateKLineInfoTooltip=function(data)
    {
        ...
        var event=this.TKChart.GetEventCallback(JSCHART_EVENT_ID.ON_FORMAT_KLINE_INFO_FLOAT_TOOLTIP);
        if (event && event.Callback)
        {
            var sendData={ AryText:aryText, Data:data, TKChart:this.TKChart };
            event.Callback(event, sendData, this);
        }
...
function JSSmallFloatTooltip()
{
    ...
    {
        var divDom=document.createElement("div");
        divDom.className='IYXChart_Small_Tooltip_Div';

        var divContent=document.createElement("div");
        divContent.className='IYXChart_Small_Tooltip_Content_Div';
...
function JSSmallFloatTooltipV2()
{
    ...
    this.ValueAlign=
    {
        Left:"IYXChart_Small_Tooltip_V2_Text2_Span",       //左对齐
        MarginLeft:'IYXChart_Small_Tooltip_V2_Text3_Span',
        Right:"IYXChart_Small_Tooltip_V2_Text_Span",
    }

    this.TitleAlign=
    {
        Default:"IYXChart_Small_Tooltip_V2_Title_Span",   //标题默认
    }
    ...
    {
        var divDom=document.createElement("div");
        divDom.className='IYXChart_Small_Tooltip_V2_Div';

        var table=document.createElement("table");
        table.className="IYXChart_Small_Tooltip_V2_Table";
        divDom.appendChild(table);

        var tbody=document.createElement("tbody");
        tbody.className="IYXChart_Small_Tooltip_V2_Tbody";
        table.appendChild(tbody);
        ...
        for(var i=0;i<this.MaxRowCount;++i)
        {
            ...
            var trDom=document.createElement("tr");
            trDom.className='IYXChart_Small_Tooltip_V2_Group_Tr';
            tbody.appendChild(trDom);
            rowItem.Tr=trDom;

            var tdDom=document.createElement("td");
            tdDom.className="IYXChart_Small_Tooltip_V2_Title_Td";   //标题
            trDom.appendChild(tdDom);
            rowItem.TitleTd=tdDom;
            ...
            var tdDom=document.createElement("td");
            tdDom.className="IYXChart_Small_Tooltip_V2_Text_Td";    //数值
            trDom.appendChild(tdDom);
            rowItem.TextTd=tdDom;

            var spanDom=document.createElement("span");
            spanDom.className='IYXChart_Small_Tooltip_V2_Text_Span';
            spanDom.innerText='数值';
            tdDom.appendChild(spanDom);
            rowItem.TextSpan=spanDom;
