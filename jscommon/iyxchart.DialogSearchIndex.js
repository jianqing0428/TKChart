...
function JSDialogSearchIndex()
{
    ...
    this.Show=function(x, y, groupID)
    {
        ...
        if (this.TKChart) this.TKChart.ClearRestoreFocusTimer();
    ...
    this.Create=function()
    {
        ...
        divDom.className='IYXChart_SearchIndex_Dialog_Div';

        //对话框标题栏
        var divTitle=document.createElement("div");
        divTitle.className='IYXChart_SearchIndex_Title_Div';
        divTitle.onmousedown=(e)=>{   this.OnMouseDownTitle(e);}
        divDom.appendChild(divTitle);

        var divName=document.createElement("div");
        divName.className='IYXChart_SearchIndex_Name_Div';
        divName.innerText="指标搜索";
        divTitle.appendChild(divName);

        var divClose=document.createElement("div");
        divClose.className='IYXChart_SearchIndex_Close_Div';
        ...
        divFrame.className="IYXChart_SearchIndex_Frome_Div";
        divDom.appendChild(divFrame);

        //搜索框
        var divInput=document.createElement("div");
        divInput.className="IYXChart_SearchIndex_Input_Div";
        divFrame.appendChild(divInput);

        var input=document.createElement("input");
        input.className='IYXChart_SearchIndex_Input';
        input.type="text";
        ...
        divContainer.className="IYXChart_SearchIndex_Container_Div";
        divDom.appendChild(divContainer);

        //分类
        var divGroup=document.createElement("div");
        divGroup.className="IYXChart_SearchIndex_GroupList_Div";
        ...
        for(var i=0, j=0;i<this.MaxGroupCount;++i)
        {
            ...
            divItem.className="IYXChart_SearchIndex_Group_Div";
            divGroup.appendChild(divItem);
            groupItem.Div=divItem;

            var spanDom=document.createElement("span");
            spanDom.className='IYXChart_SearchIndex_Group_Span';
        ...
        divTable.className='IYXChart_SearchIndex_Table_Div';
        divContainer.appendChild(divTable);
        
        var table=document.createElement("table");
        table.className="IYXChart_SearchIndex_Table";
        divTable.appendChild(table);

        var tbody=document.createElement("tbody");
        tbody.className="IYXChart_SearchIndex_Tbody";
        ...
        for(var i=0, j=0;i<this.MaxRowCount;++i)
        {
            ...
            trDom.className='IYXChart_SearchIndex_Group_Tr';
    ...
    this.CreateCellDOM=function(rowID, colID, trDom)
    {
        ...
        tdDom.className="IYXChart_SearchIndex_Text_Td";   //指标名称
        trDom.appendChild(tdDom);
        cellItem.Td=tdDom;

        var spanDom=document.createElement("span");
        spanDom.className='IYXChart_SearchIndex_Text_Span';
    ...
    this.OnClickIndex=function(e, cellItem)
    {
        ...
        var event=this.TKChart.GetEventCallback(JSCHART_EVENT_ID.SEARCH_DIALOG_ON_CLICK_INDEX);
        if (event && event.Callback)
        {
            var sendData={ OpData:this.OpData, IndexData:cellItem.IndexItem, TKChart:this.TKChart, PreventDefault:false };
        ...
        if (this.OpData.OpType==1)
        {
            ...
            {
                this.TKChart.ChangeIndex(this.OpData.WindowIndex, indexItem.ID, indexItem);
            }
            else if(indexItem.Type==4 || indexItem.Type==5) //五彩K线
            {
                this.TKChart.ChangeInstructionIndex(indexItem.ID);
            }
            else if (indexItem.Type==1) //自定义脚本指标
            {
                ...
                this.TKChart.ChangeScriptIndex(this.OpData.WindowIndex, indexData);
            }
            else if (indexItem.Type==2) //api指标
            {
                var indexData={ API: { ID:indexItem.ID, Name:indexItem.Name, Args:indexItem.Args, Url:'local'} };
                if (indexItem.Lock) indexData.Lock=indexItem.Lock;
                this.TKChart.ChangeAPIIndex(this.OpData.WindowIndex, indexData);
            }
            else if (indexItem.Type==3) //指标模板
            {
                this.TKChart.ChangeIndexTemplate(indexItem.TemplateData)
            }
        }
        else if (this.OpData.OpType==2)
        {
            ...
            if (indexItem.Type==0)  //系统指标
            {
                ...
                this.TKChart.AddOverlayIndex(obj);
            }
            else if (indexItem.Type==1) //自定义脚本指标
            {
        ...
        else if (this.OpData.OpType==3) //新增加指标窗口
        {
            ...
            if (indexItem.Type==0)  //系统指标
            {
                this.TKChart.AddIndexWindow(indexItem.ID, this.OpData);
            }
            else if (indexItem.Type==1) //自定义脚本指标
            {
                var indexData={ ID:indexItem.ID, Name:indexItem.Name, Script:indexItem.Script, Args:indexItem.Args };
                this.TKChart.AddScriptIndexWindow(indexData, this.OpData);
            }
            else if (indexItem.Type==2) //api指标
            {
                var indexData={ API: { ID:indexItem.ID, Name:indexItem.Name, Args:indexItem.Args, Url:'local'} };
                if (indexItem.Lock) indexData.Lock=indexItem.Lock;
                this.TKChart.AddAPIIndexWindow(indexData, this.OpData);
            }
            else if (indexItem.Type==3) //指标模板
            {
                this.TKChart.ChangeIndexTemplate(indexItem.TemplateData)
...
function JSDialogModifyIndexParam()
{
    ...
    {
        var divDom=document.createElement("div");
        divDom.className='IYXChart_ModifyIndexParam_Dialog_Div';

        //对话框标题栏
        var divTitle=document.createElement("div");
        divTitle.className='IYXChart_ModifyIndexParam_Title_Div';
        divTitle.onmousedown=(e)=>{   this.OnMouseDownTitle(e);}
        divDom.appendChild(divTitle);

        var divName=document.createElement("div");
        divName.className='IYXChart_ModifyIndexParam_Name_Div';
        divName.innerText="指标参数修改";
        divTitle.appendChild(divName);

        var divClose=document.createElement("div");
        divClose.className='IYXChart_ModifyIndexParam_Close_Div';
        divClose.innerText="x";
        divClose.onmousedown=(e)=>{ this.OnClickColseButton(e); }
        divTitle.appendChild(divClose);
        ...
        //整个框子
        var divFrame=document.createElement("div");
        divFrame.className="IYXChart_ModifyIndexParam_Frome_Div";
        divDom.appendChild(divFrame);

        //表格
        var divTable=document.createElement("div");
        divTable.className='IYXChart_ModifyIndexParam_Table_Div';
        divFrame.appendChild(divTable);
        
        var table=document.createElement("table");
        table.className="IYXChart_ModifyIndexParam_Table";
        divTable.appendChild(table);

        var tbody=document.createElement("tbody");
        tbody.className="IYXChart_ModifyIndexParam_Tbody";
        table.appendChild(tbody);
        ...
        divButton.className='IYXChart_ModifyIndexParam_Button_Div';
        divFrame.appendChild(divButton);

        if (this.EnableRestoreParam)
        {
            var btnRestore=document.createElement("button");
            //btnRestore.className="IYXChart_ModifyIndexParam_button";
        ...
        //btnOk.className="IYXChart_ModifyIndexParam_button";
        btnOk.innerText="确认";
    ...
    this.CreateRowDOM=function(index, tbody)
    {
        ...
        var trDom=document.createElement("tr");
        trDom.className='IYXChart_ModifyIndexParam_Tr';
        tbody.appendChild(trDom);
        rowItem.Tr=trDom;

        var tdDom=document.createElement("td");
        tdDom.className="IYXChart_ModifyIndexParam_Name_Td";   //指标名称
        trDom.appendChild(tdDom);
        rowItem.TdName=tdDom;

        var spanDom=document.createElement("span");
        spanDom.className='IYXChart_ModifyIndexParam_Name_Span';
        spanDom.innerText='参数';
        tdDom.appendChild(spanDom);
        rowItem.SpanName=spanDom;

        var tdDom=document.createElement("td");
        tdDom.className="IYXChart_ModifyIndexParam_Value_Td";   //指标名称
        trDom.appendChild(tdDom);
        rowItem.TdValue=tdDom;

        var input=document.createElement("input");
        input.className='IYXChart_ModifyIndexParam_Input';
        input.type="number";
        input.step=1;
    ...
    this.RestoreParam=function()
    {
        ...
        if (this.IndexData.Type==1) this.TKChart.UpdateWindowIndex(this.IndexData.WindowIndex);
        else if (this.IndexData.Type==2) this.TKChart.UpdateOverlayIndex(this.IndexData.Identify);
    }
    ...
    this.GetDefaultParam=function()
    {
        ...
        var event=this.TKChart.GetEventCallback(JSCHART_EVENT_ID.GET_DEFAULT_INDEX_PARAM);
        if (event && event.Callback)
        {
    ...
    */
    ...
    {   
        ...
        {
            this.TKChart.UpdateWindowIndex(this.IndexData.WindowIndex);
        }
        else if (this.IndexData.Type==2) 
        {
            this.TKChart.UpdateOverlayIndex(this.IndexData.Identify);
        }
    }
}
