...
HQData.NetworkFilter=function(data, callback)
{
    ...
    switch(data.Name) 
    {
        ...
        case "JSSymbolData::GetFinance":    //财务数据
            //TKChart使用教程30-K线图如何对接第3方数据23- FINANCE函数数据
        ...
        case "JSSymbolData::GetFinValue":
            //TKChart使用教程30-K线图如何对接第3方数据22- FINVALUE函数数据
        ...
        case "JSSymbolData::GetIndexData":
            //TKChart使用教程30-K线图如何对接第3方数据28-大盘数据[INDEXA,INDEXC......]
        ...
        case 'JSSymbolData::GetSymbolData':   
            //TKChart使用教程30-K线图如何对接第3方数据38-通达信指标K线数据
...
HQData.Minute_RequestMinuteData=function(data, callback)
{
    ...
    setTimeout(()=>{
        ...
        */
        ...
        var tkchartData={code:0, stock:[stockItem] };

        var time=new Date();
        if (time.getSeconds()%3==1)
            tkchartData.LatestPointFlash={ FlashCount:2 };
    

        callback(tkchartData);
    }, 50);
}
...
HQData.Minute_RequestMinuteUpdateData=function(data, callback)
{
    ...
    setTimeout(()=>
    {
        ...
        var tkchartData={code:0, stock:[stockItem], dataType:1 };
        tkchartData.LatestPointFlash={ FlashCount:2 };
...
HQData.KLine_RequestMulitDayMinuteData=function(data, callback)
{
    ...
    var tkchartData={ name:symbol, symbol:symbol, data:aryKLine, ver:2.0, AryBreakPoint:aryBreakPoint };
    callback(tkchartData);
}
...
HQData.Minute_RequestHistoryMinuteData=function(data, callback)
{
    ...
    var tkchartData={code:0, data:aryDay, name:symbol, symbol: symbol};

    //tkchartData.data[0].minute.length=45;
...
HQData.RequestHistoryData=function(data,callback)
{
    ...
    var tkchartData={ name:symbol, symbol:symbol, data:aryData, ver:2.0 };
...
HQData.RequestFlowCapitalData=function(data,callback)
{
    ...
    var tkchartData=KLINE_CAPITAL_DATA;
    callback(tkchartData);
}
...
HQData.RequestRealtimeData=function(data, callback)
{
    ...
    var aryStock=[];
    var tkchart=data.Self;
    var kData=tkchart.GetKData();
    ...
    if (bVer3)
    {
        ...
        var tkchartData={ code:0, stock:aryStock, Ver:3.0 };
    }
    else
    {
        for(var i=0;i<data.Request.Data.symbol.length;++i)
        ...
        var tkchartData={ code:0, stock:aryStock, };
    }
...
HQData.RequestMinuteRealtimeData=function(data,callback)
{
    ...
    var symbol=data.Request.Data.symbol[0]; //请求的股票代码
    var tkchartData={ name:symbol, symbol:symbol, ver:2.0, data:aryMainData };
    if (IFrameSplitOperator.IsNonEmptyArray(aryOverlay)) tkchartData.overlay=aryOverlay;
...
HQData.RequestKLineTickData=function(data,callback)
{
    ...
    var tkchartData={ code:0, symbol:symbol, name:symbol, data:[], ver:2.0 };
...
HQData.RequestIndexVariantData=function(data,callback)
{
    ...
    if (varName=="FROMOPEN") 
    {
        var tkchartData={  };
        //单数据
        tkchartData.Data={ Date:20230707, Value:240 };
        tkchartData.DataType=1;
        callback(tkchartData);
    }
    else if (varName=="FGBLOCK")
    {
        var tkchartData={ DataType:1, Data:{Value:"融资融券 大盘股 MSCI成份 周期股 沪股通标的"} }; //返回所属风格板块.
        callback(tkchartData);
    }
    else if (varName=="GNBLOCK")
    {
        var tkchartData={ DataType:1,  Data:{ Value:"含可转债 跨境支付CIPS"} }; //返回所属概念板块.
        callback(tkchartData);
    }
    else if (varName=="HYBLOCK")
    {
        var tkchartData={ DataType:1,  Data:{Value:"全国性银行"}}; //返回品种所属行业.
        callback(tkchartData);
    }
    else if (varName=="DYBLOCK")
    {
        var tkchartData={ DataType:1, Data:{Value:"上海板块"} }; //返回品种所属地域..
        callback(tkchartData);
    }
    else if (varName=="CAPITAL")
    {
        var tkchartData={ DataType:1, Data:{ Value:29352177375 } }; // 当前流通股本,单位为手,债券1手为10张,其它为100
        callback(tkchartData);
    }
    else if (varName=="LARGEINTRDVOL")
    {
        ...
        var tkchartData={ DataType:2, Data:[] };
        var testValue=0.15;
        for(var i=0;i<kData.Data.length;++i)
        {
            var kItem=kData.Data[i];
            tkchartData.Data.push({ Date:kItem.Date, Time:kItem.Time, Value:kItem.Vol*testValue });
            testValue+=0.01;
    ...
    else if (varName=="LARGEOUTTRDVOL")
    {
        ...
        {
            var kItem=kData.Data[i];
            tkchartData.Data.push({ Date:kItem.Date, Time:kItem.Time, Value:kItem.Vol*0.17 });
        }
        callback(tkchartData);
    }
    ...
    {
        var kData=data.Self.Data;
        var tkchartData={ DataType:0, Data:[], Ver:2.0 };
        for(var i=0;i<kData.Data.length;++i)
        {
            var kItem=kData.Data[i];
            var value=HQData.GetRandomTestData(1,10);
            tkchartData.Data.push({ Date:kItem.Date, Time:kItem.Time, Value:value });
        }
...
HQData.CustomFunction_RequestData=function(data, callback)
{
    ...
    if (funcName=='L2_VOL')
    {
        ...
        {
            var kItem=kData.Data[i];
            tkchartData.Data.push({ Date:kItem.Date, Time:kItem.Time, Value:kItem.Vol/3 });
        }
    }
    else if (funcName=="FUNC_ALPHA")
    {
        ...
        setTimeout(()=>
        {
            ...
            {
                var kItem=kData.Data[i];
                tkchartData.Data.push({ Date:kItem.Date, Time:kItem.Time, Value:HQData.GetRandomTestData(0,100)/100 });
            }
             callback(tkchartData);
        },300);
...
HQData.CustomVarData_RequestData=function(data, callback)
{
    ...
    if (varName=="DCLOSE")
    {
        ...
        {
            var kItem=kData.Data[i];
            tkchartData.Data.push({ Date:kItem.Date, Time:kItem.Time, Value:kItem.Close });
        }
...
HQData.RequestOtherSymbolData=function(data, callback)
{
    ...
    var tkchartData={ name:symbol, name:symbol, data:aryData };
    callback(tkchartData);
}
...
HQData.AnnouncementInfo_RequestData=function(data,callback)
{
    ...
    var tkchartData={ symbol:symbol, report:[] };

    var kData=data.TKChart.ChartPaint[0].Data;
    for(var i=0, j=1;i<kData.Data.length;++i)
    {
        ...
        tkchartData.report.push(itemReport);
...
HQData.ResearchInfo_RequestData=function(data,callback)
{
    ...
    var tkchartData={ symbol:symbol, list:[] };
    ...
    for(var i=0, j=1;i<kData.Data.length;++i)
    {
        ...
        tkchartData.list.push(itemReport);
...
HQData.RequestLatestData=function(data,callback)
{
    ...
    var tkchartData={ symbol:symbol, ver:2.0, data:[ {id:id, value:value }]};
...
HQData.RequestOverlayHistoryData=function(data, callback)
{
    ...
    var tkchartData={  code:0, symbol: symbol,name: symbol, ver:2.0, data:aryData };
...
HQData.RequestOverlayHistoryMinuteData=function(data, callback)
{
    ...
    var tkchartData={  symbol: symbol,name: symbol, ver:2.0, data:aryData };
...
HQData.Finance_RequestData=function(data,callback)
{
    ...
    {
        tkchartData=TEST_FINANCE_7.data;
    }
    else 
    {
        var error= `Finance(${id}) 没有对接数据. [HQData.Finance_RequestData]`;
        tkchartData={ Error:error }; 
    }

    if (tkchartData) callback(tkchartData);
}
...
HQData.INDEX_RequestData=function(data,callback)
{
    ...
    var tkchartData={ name:indexSymbol, symbol:indexSymbol, data:aryData, ver:2.0 };
...
HQData.Report_RequestStockListData=function(data, callback)
{
    ...
    if (IFrameSplitOperator.IsNonEmptyArray(SHSZ_STOCK_LIST_TEST_DATA.symbol))
    {
        ...
        for(var i=0;i<SHSZ_STOCK_LIST_TEST_DATA.symbol.length;++i)
        {
            ...
            //item[88]=1;
            tkchartData.data.push(item);
        }
    }

    console.log("[HQData.Report_RequestStockListData] tkchartData",tkchartData);
    callback(tkchartData);
...
HQData.Report_RequestStockListData_Empty=function(data, callback)          
{
    data.PreventDefault=true;
    var tkchartData={ data:[] };
    console.log("[HQData.Report_RequestStockListData_EMPTY] tkchartData",tkchartData);
    callback(tkchartData);
}
...
HQData.Report_RequestMemberVirtualListData=function(data, callback, option)
{
    ...
    var tkchartData= { symbol:symbol , name:symbol, data:[] , code:0, Virtual:{ Count:option.Virtual.Count } };

    for(var i=0; i<SHSZ_STOCK_LIST_TEST_DATA.symbol.length && i<50; ++i )
    {
        tkchartData.data.push(SHSZ_STOCK_LIST_TEST_DATA.symbol[i]);
    }
...
HQData.Report_RequestMemberListDat=function(data, callback)
{
    ...
    var tkchartData= { symbol:symbol , name:symbol, data:[] , code:0 };
...
HQData.Report_RequestStockData=function(data, callback)
{
    ...
    setTimeout(()=>
    {
        ...
        for(var mapItem of mapStock)
        {
            ...
            tkchartData.data.push(item.Data);
        }
...
HQData.Report_RequestStockSortData=function(data, callback)
{
    ...
    var tkchartData={ data:[], index:[], filedindex:column.index, sort:sortType, symbol:blockID };
    ...
    for(var mapItem of mapStock)
    {
        ...
        tkchartData.data.push(item.Data);
        tkchartData.index.push(item.Index);
    }
    ...
    if (IFrameSplitOperator.IsNonEmptyArray(SHSZ_STOCK_LIST_TEST_DATA.symbol))
    {
        ...
        for(var i=0;i<count;++i)
        {
           ...
           if (i%20==3) newItem[80]='rgb(132,112,255)';
            ...
            if (i==0)
            {
                tkchart.SetFlashBGItem(symbol, { ID:-1 , Color:"rgb(200,0,200)" });
            }
        }
    }

    var tkchartData={ data:aryData, index:aryIndex, filedindex:-1, sort:sortType, symbol:blockID };

    if (column)  tkchartData.filedindex=column.index;
...
HQData.APIIndex_MULTI_POINT=function(data, callback)
{
    ...
    var apiData=
    { 
        code:0, stock:{ name:tkchart.Name, symbol:tkchart.Symbol }, 
...
HQData.APIIndex_DRAWTEXTREL=function(data, callback)
{
    ...
    {
        code:0, 
        stock:{ name:tkchart.Name, symbol:tkchart.Symbol }, 
...
HQData.APIIndex_DRAWCOLORKLINE=function(data, callback)
{
    ...
    console.log("[HQData.APIIndex_DRAWCOLORKLINE] DRAWCOLORKLINE=",tkchartData);
...
HQData.APIIndex_KLINE_TABLE=function(data, callback)
{
    data.PreventDefault=true;
    var tkchart=data.TKChart;
    var kData=data.TKChart.GetKData(); //tkchart图形的分钟数据
...
HQData.RequestVolumeProfileData=function(data, callback)
{
    ...
    var tkchartData={ Data:aryData, MaxPrice:maxPrice, MinPrice:minPrice, PriceOffset:step, code:0 };

    console.log('[HQData.RequestVolumeProfileData] tkchartData=', tkchartData);
...
{
    data.PreventDefault=true;
    var tkchartData={ outdata:{ date:null, time:null, outvar:[] } , code:0};
    tkchartData.error={ message:"错误提示信息" };
    callback(tkchartData);
}
