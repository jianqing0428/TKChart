...
function JSPopMinuteChart()
{
    ...
    this.Inital=function(tkchart, option)
    {
        ...
        if (option)
        {
            ...
            {
                this.TKChart.CreateExtendChart("MarkPopMinutePaint");
            }
        }
    }
    ...
    this.Create=function()
    {
        ...
        //语言跟主图保持一致
        if (this.TKChart) this.Minute.Option.Language=g_JSChartLocalization.GetLanguageName(this.TKChart.LanguageID);
        this.Minute.Option.OnCreatedCallback=(chart)=>{ this.OnCreateTKChart(chart); }
    ...
    {
        if (!this.TKChart || !this.TKChart.NetworkFilter) return;
        ...
        if (data.Name== 'MinuteChartContainer::RequestMinuteData')  //分时图数据对接
        {
            ...
            this.TKChart.NetworkFilter(data, (recv)=>{ this.OnRecvPopMinuteData(recv, callback); });   
        }
        else
        {
            this.TKChart.NetworkFilter(data, callback);   
        }
    }
    ...
    this.MarkKLineBG=function()
    {
        ...
        var finder=this.TKChart.GetExtendChartByClassName("MarkPopMinutePaint");
...
function JSTooltipMinuteChart()
{
    ...
    {
        var divDom=document.createElement('div');
        divDom.className='IYXChart_Tooltip_Minute_Div';
        divDom.id=this.ID;

        var divChart=document.createElement('div');
        divChart.className='IYXChart_Tooltip_Minute_Chart_Div';
        divDom.appendChild(divChart);
...
function MarkPopMinutePaint()
{
    ...
    this.Draw=function()
    {
        ...
        var kData=this.TKChart.GetKData();
